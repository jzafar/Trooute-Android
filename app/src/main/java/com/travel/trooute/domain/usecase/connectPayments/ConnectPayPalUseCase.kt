...
class ConnectPayPalUseCase @Inject constructor(private val repository: ConnectPaymentsRepository) {
    suspend operator fun invoke(request: ConnectPaypalRequest): Resource<AuthResponse> {
        return repository.connectPayPal(request = request)
    }
}